...
def main() -> int:
    ...
    # Права НЕ сужаем до 0600, и это не небрежность. Файл монтируется в
    # контейнер, где процесс работает под uid 10001, а создаёт файл пользователь
    # раннера с другим uid: владелец-только означает, что сервис его не
    # прочитает. Проявляется это не ошибкой доступа, а `degraded` в реестре
    # ключей и `503` на каждой загрузке — то есть далеко от причины.
    #
    # Секрета здесь и нет: он сгенерирован, живёт минуты и не выходит за
    # пределы сети стенда. В бою правило другое — см. `make config-check`.
    ...
    # Та же причина, что и у ключей: читает notifier под uid 10001.
    target.chmod(0o644)
