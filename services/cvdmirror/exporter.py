...
def collect(directory: Path = DB_DIR) -> None:
    ...
    if not found:
        # Пустой каталог — это работающая раздача без единой базы.
        #
        # Ряд обязан существовать: отсутствие неотличимо от неработающего
        # экспортёра. Но значение — бесконечность, а не ноль. Ноль у ВОЗРАСТА
        # означает «обновлено только что», и так было: пустое зеркало в
        # кластере отчитывалось самой свежей базой из возможных, а любое
        # правило `возраст > порога` на нём молчало. Правду говорит только
        # бесконечность — баз нет, то есть они бесконечно стары.
        newest.set(math.inf)
        return
