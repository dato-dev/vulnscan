...
class NotifierSettings(CommonSettings):
    ...
    """Выгрузка обезвреженных копий (M14.1). Отдельная очередь от коллбэков:
    доставка файла в чужое хранилище упирается в чужую сеть и может тянуться
    минутами, а коллбэк — это один HTTP-запрос."""

    delivery_credentials_file: str | None = None
    """Учётные данные приёмников обезвреженных копий (M14.8).

    Отдельный файл, а не поле в политике, и лежит он не в общем каталоге
    конфигурации. Причина: `policies.json` читают gateway, воркер и deepscan —
    в том числе процесс, разбирающий враждебные файлы. Адрес приёмника ему
    безвреден, ключ доступа к чужому хранилищу — нет.

    Пусто — доставки нет, и это обычное состояние установки.
    """
