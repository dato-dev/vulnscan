...
class WorkerSettings(CommonSettings):
    ...
    yara_candidate_dir: str = ""
    """Набор-кандидат: прогоняется вхолостую рядом с действующим (M7.2).

    Пусто — канарейки нет, и это обычное состояние. Каталог задают на время
    выкатки: кандидат считает, что нашёл бы, но на вердикт, `engines` и кэш не
    влияет. Набор, способный изменить вердикт, — не канарейка, а выкатка на
    долю трафика; узнать цену выкатки должно быть безопасно.

    Цена включения — второй проход YARA на файл, 5-50 мс.
    """
