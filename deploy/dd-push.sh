...
[ "$fail" = 0 ] || { echo; echo "Не готово к запуску."; exit 1; }
...
case "$probe" in
    ...
    000)     bad "API не отвечает"; sed 's/^/      /' "$probe_err" ;;
...
run_trivy() {  # режим цель формат файл метка [доп. флаги...]
    mode=$1; target=$2; fmt=$3; out=$4; label=$5
    shift 5
    ...
    rm -f "$out.err"
    [ -s "$out" ] || { bad "$label: trivy вернул пустой отчёт"; return 1; }
    say "$label"
}
...
sbom() {  # режим цель продукт service имя-файла метка
    [ "$DD_SBOM" = "1" ] || return 0
    mode=$1; target=$2; product=$3; service=$4; name=$5; label=$6
...
scan_sast() {
    ...
    set -- $cmd scan --quiet --metrics=off --json --output "$out"
    for cfg in $DD_SEMGREP_CONFIG; do set -- "$@" --config "$cfg"; done
    set -- "$@" .
    run_sast "semgrep — $DD_SEMGREP_CONFIG" "$out" "$@" \
