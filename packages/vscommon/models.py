...
class TenantPolicy(BaseModel):
    ...
    deliver_blocked: Literal["never", "strict"] = "never"
    """Отдавать ли пересобранную копию заблокированного файла (M14.9).
    ...
    `strict` — пересобрать растеризацией и отдать. Здесь утверждение держится:
    из исходника не остаётся ни одного объекта, поэтому оно не зависит от
    того, что в нём было. Цена — теряется текстовый слой, документ становится
    картинками.

    Промежуточных значений нет намеренно. `standard` для заблокированного
    выглядел бы как разумный компромисс и был бы худшим из вариантов:
    эвристика, выданная за гарантию.
    """
...
class DeliveryTask(BaseModel):
    ...
    """Копия пересобрана из ЗАБЛОКИРОВАННОГО файла (M14.9).

    Уезжает в отдельный каталог и помечается в манифесте. Смешивать такие
    копии с обычными нельзя: скрипт на стороне клиента обрабатывает каталог
    целиком и манифест у каждого файла не читает.
    """
...
class ScanJob(BaseModel):
    ...
    size: int
    filename: str | None = None
    """Имя исходного файла — ТОЛЬКО когда его просит шаблон приёмника (M14.10).

    По умолчанию `None`, и это не мелочь: задание лежит в Redis, а Redis пишет
    на диск. Имена документов часто содержат ФИО и номера, поэтому gateway
    оставляет от имени расширение и выбрасывает остальное. Несём его лишь
    тогда, когда тенант явно попросил `{filename}` в `key_template`, — иначе
    хранение персональных данных, включённое «на всякий случай», остаётся
    навсегда.
    """
