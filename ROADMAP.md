...
Восемь майлстоунов из четырнадцати закрыты. Сервис работает на боевом стенде:
...
**M11** (выкат в Kubernetes), **M13** (сквозное тестирование), **M14**
(доставка копии во внешнее хранилище) и два долга — **D4** и **D7**.

**D7 стоит починить раньше остальных долгов**: `deploy/config` монтируется в
воркер целиком, вместе с `keys.json`. Секреты тенантов читаются из процесса,
который разбирает враждебные файлы, — изоляция держится на незаданной
переменной окружения, а не на правах доступа.

**M14 — про бизнес-модель, а не про транспорт.** Сегодня копию забирают у нас,
и сутки мы держим чужие документы. Складывая её сразу в хранилище клиента, мы
перестаём хранить их вовсе — и берём взамен другой риск: постоянный доступ на
запись в чужую инфраструктуру. Поэтому там же решается форма этого доступа, а
не только протокол.
...
**Первым в M13 идёт доставка копий из M14** (пункт M13.0). Её путь ни разу не
выполнялся против настоящего S3, и это единственный путь в сервисе, где отказ
виден не нам, а клиенту — в его собственном пустом ящике.
...
**Первый кандидат — доставка копий (M14).** Весь её путь ни разу не выполнялся
против настоящего S3: `boto3` в тестах подменён, Docker при разработке был
недоступен. Проверено устройство и границы — что приёмник берётся из политики,
что манифест уезжает вторым, что учётка нужна только на запись, — но не сама
выгрузка. До первого живого прогона считать доставку рабочей нельзя.
...
- [ ] **M13.0** Доставка обезвреженной копии против настоящего S3 (MinIO в
      compose как «чужое» хранилище). Первым, раньше каркаса для остального:
      путь новый, ни разу не выполнявшийся, и заглушка на нём проверяет наши
      представления о boto3, а не работу.
      *Что должно подтвердиться: файл и манифест появляются под нужным
      префиксом; манифест уезжает ВТОРЫМ; заблокированный файл даёт манифест
      без файла; недоступный приёмник даёт повтор, а затем dead-letter с
      причиной `delivery_failed`; **учётной записи с правами только на запись
      хватает** — это единственный способ проверить M14.4, потому что права
      выдаёт хранилище, а не наш код.*
      *Критерий: прогон падает, если убрать любую из этих гарантий.*
...
- [x] **M14.1** Внешний S3 как приёмник.
      ...
      *Имя объекта — `scan_id` с расширением. Исходное имя не используется: оно
      часто содержит персональные данные, и мы его не храним.*
      ...
      ***Против настоящего S3 не выполнялось ни разу.** `boto3` в тестах
      подменён, Docker при разработке был недоступен. Проверено устройство и
      границы, но не сама выгрузка: см. M13.0 — это первый кандидат на
      сквозной прогон, и до него доставку рабочей считать нельзя.*
