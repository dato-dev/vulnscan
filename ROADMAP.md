...
Шесть майлстоунов из десяти закрыты, M4 переоткрыт: наблюдаемость работает, но
версии библиотек и образов отстали от обновлённого стека мониторинга. Сервис работает на боевом стенде: принимает
файлы от Telegram-бота, отдаёт вердикт и пересобранную копию, ведёт историю в
PostgreSQL, отдаёт трассировки и метрики в общий стек наблюдаемости.

Открыты **M4** (три пункта про версии), **M6** (офисные форматы и архивы),
**M7** (эксплуатация), **M9** (SDK для сторонних команд), **M10** (метрики
целиком: контракт, стек, сервисы, панели) и один долг — **D4**.
...
- [ ] **M4.16** Поднять версии телеметрии вслед за стеком мониторинга.
      ...
      *Версии живут в ДВУХ местах: `pyproject.toml` и строка `pip install` в
      каждом Dockerfile. Тесты гоняются по первому, образы собираются по
      второму — расхождение проявится только на боевом стенде. Свести к одному
      источнику важнее самого бампа.*
      *Критерий: `make test` зелёный, трейсы и метрики доходят после обновления.*

- [ ] **M4.17** Привести версии образов в compose к стеку.
...
- [ ] **M9.1** Документация SDK: справочник по методам, разбор вердикта,
      ...
      *Критерий: подключиться можно, не открывая исходники библиотеки.*

- [ ] **M9.2** Спецификация протокола отдельным документом.
      ...
      *Критерий: по документу можно написать клиента, не читая наш код.*

- [ ] **M9.3** Набор тестов совместимости, общий для всех реализаций.
      ...
      *Критерий: реализация на любом языке проходит один и тот же набор.*

- [ ] **M9.4** SDK на JavaScript/TypeScript.
      *Первый после Python: формы обратной связи и Node-бэкенды — самый
      вероятный сценарий подключения сторонней команды.*
      *Внимание к деталям: в браузере ключ держать нельзя, библиотека должна
      быть серверной и не притворяться иначе.*
...
- [ ] **M10.1** Список метрик как контракт, а не как побочный результат.
      ...
      *Критерий: по документу видно, какой метрики не хватает, — без чтения кода.*

- [ ] **M10.2** Свести RED/USE к вердикту, а не к процессам.
      *Сегодня хорошо видно, что делает сервис, и хуже — что получает клиент.
      Доля `unsupported`, доля деградации, время до вердикта по классам файлов
      важнее, чем загрузка воркера.*

- [ ] **M10.3** Метрики на всё, что деградирует молча.
      *Свежесть баз ClamAV, версия правил YARA, попадания в кэш, отставание
      writer, глубина очереди коллбэков. Каждое из этого умеет молча
      испортиться — и один раз уже испортилось.*

### Стек (`deploy/lgtp`)

- [ ] **M10.4** Тест на покрытие целей, а не только на дашборды.
      *`tests/test_dashboards.py` проверяет, что панели ссылаются на
      ...
      там пропали цели.*
      *Критерий: удаление любого `job_name` роняет тест.*

- [ ] **M10.5** Проверять конфигурацию стека до выката, а не после.
      *`promtool check config`, `promtool check rules`, валидация коннекторов
      коллектора. Коллектор уже падал на `duplicate dimension name`, и узнали
      мы это из логов упавшего контейнера на сервере.*
...
- [ ] **M10.6** Алерты довести до адресата и проверить доставку.
      ...
      *Заодно: `alertmanager/alertmanager.yml` и `alloy/config.alloy` в
      репозитории отсутствуют, хотя compose их монтирует. Отсутствующий файл
      Docker создаёт каталогом — на этот способ сломаться мы наступали трижды.*

- [ ] **M10.7** Развести, что считает коллектор, а что сервисы.
      *`traces_span_metrics_*` из коллектора и наши `vs_*` частично меряют одно
      и то же разными способами. Совпадать они не обязаны, но расхождение
      должно быть объяснимым, а не обнаруживаться на панели.*

### Сервисы

- [ ] **M10.8** Единая договорённость об именах и метках.
      *Префикс, единицы (секунды, не миллисекунды), `_total` у счётчиков,
      набор меток. Сейчас это соблюдается по памяти.*

- [ ] **M10.9** Порт метрик — по одному правилу для всех сервисов.
      *Бот молча не скрейпился, потому что глобальный `METRICS_PORT` из `.env`
      перебил его собственный дефолт. Порт задаётся в трёх местах: код,
      compose, конфигурация Prometheus, — и они расходятся молча.*

- [ ] **M10.10** Метрики демо-сайта и cvdmirror.
      *Оба вне наблюдения. У зеркала это существенно: неудачное обновление баз
      выглядит как обычная работа, а деградация проявится только вердиктом.*

- [ ] **M10.11** Пройти по стадиям и добрать недостающее.
      *Тайминги стадий есть, но нет распределения по причинам отказа и по
      размеру входа — а именно они объясняют, почему p95 уехал.*

### Дашборды

- [ ] **M10.12** Пересобрать панели под M10.1, а не наоборот.
      *Три дашборда писались от доступных метрик. После того как список метрик
      станет контрактом, часть панелей окажется лишней, а часть вопросов —
      неотвеченной.*

- [ ] **M10.13** Дашборд дежурного: одна страница, отвечающая «всё ли хорошо».
      *Сейчас, чтобы понять состояние, надо открыть три дашборда и знать, куда
      смотреть.*

- [ ] **M10.14** Панель для каждого алерта.
      *Сработавший алерт должен вести на картинку, где видно, что произошло.
      Сейчас связи нет, и разбор начинается с поиска нужной панели.*

- [ ] **M10.15** Проверить дашборды на пустых данных.
      *Пустая панель и панель со сломанным запросом выглядят одинаково. Как
      минимум — описания, из которых понятно, что означает пустота.*
