...
- [x] **M4.16** Поднять версии телеметрии вслед за стеком мониторинга.
      ...
      *Сделано. Версий было не два места, а семь: `pyproject.toml` и шесть
      ...
      внутри контейнера.*
      *Теперь `requirements-telemetry.txt` (OTel 1.44.0) и
      `requirements-metrics.txt` (prometheus-client 0.26.0) — точные
      версии, а не границы. Метрики отдельным файлом: они нужны и там, где OTel
      ни при чём, — зеркалу баз и примеру подключения.*
      *Проверяется тремя тестами: в файлах нет `>=`, Dockerfile не называет
      версию сам, pyproject совпадает с закреплением и с установленным
      окружением.*
